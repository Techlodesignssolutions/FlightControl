...
Attitude StateEstimator::quaternionToEuler(float q0, float q1, float q2, float q3) {
    ...
    attitude.roll = std::atan2(sinr_cosp, cosr_cosp);
    
    // Pitch (y-axis rotation)
    float sinp = 2 * (q0 * q2 - q3 * q1);
    if (std::abs(sinp) >= 1)
        attitude.pitch = std::copysign(0.5f * M_PI, sinp); // Use +/-pi/2 if out of range
    else
        attitude.pitch = std::asin(sinp);
    
    // Yaw (z-axis rotation)
    float siny_cosp = 2 * (q0 * q3 + q1 * q2);
    float cosy_cosp = 1 - 2 * (q2 * q2 + q3 * q3);
    attitude.yaw = std::atan2(siny_cosp, cosy_cosp);
