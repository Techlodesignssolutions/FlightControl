...
bool FlightGearHAL::readIMU(float* gyro_xyz, float* accel_xyz, float* mag_xyz) {
    ...
    // Gyroscope output is rad/s by HAL contract.
    gyro_xyz[0] = addNoise(degreesToRadians(fg_data_.roll_rate_degps) + gyro_bias_[0], 0.02f);   // Roll rate
    gyro_xyz[1] = addNoise(degreesToRadians(fg_data_.pitch_rate_degps) + gyro_bias_[1], 0.02f);  // Pitch rate
    gyro_xyz[2] = addNoise(degreesToRadians(fg_data_.yaw_rate_degps) + gyro_bias_[2], 0.02f);    // Yaw rate
